...
try:
    ...
    print("[OK] Using Real Zonos TTS implementation")
    USING_REAL_ZONOS = True
except ImportError as e:
    print(f"[WARNING] Real Zonos not available ({e}), using placeholder implementation")
...
app = FastAPI(title="Zonos TTS Microservice", version="1.0.0")
...
async def startup_event():
    ...
    try:
        ...
        logging.error(f"[ERROR] Zonos TTS initialization failed: {e}")
        raise
...
async def synthesize_speech(request: SynthesizeRequest) -> SynthesizeResponse:
    ...
    try:
        ...
        audio_bytes = await tts_service.synthesize_speech(**synthesis_params)
...
async def get_available_voices():
    ...
    try:
        options = tts_service.get_available_options()
...
if __name__ == "__main__":
    ...
    def signal_handler(sig, frame):
        ...
        logging.info(f"[SHUTDOWN] Zonos TTS service received signal {sig}")
        sys.exit(0)
