...
try:
    from ws_service_registry import StreamingMessage, ServiceRegistration, ServiceCapabilities, MessageType
...
try:
    from voicebot_orchestrator.zonos_tts_service import ZonosTTSService
...
class WebSocketZonosTTSService:
    def __init__(self, orchestrator_url="ws://localhost:9001"):
        ...
        if TTS_AVAILABLE:
            try:
                self.zonos_tts = ZonosTTSService()
    ...
    async def register_service(self):
        ...
        try:
            registration_message = StreamingMessage(
                session_id=self.service_id,
                timestamp=time.time(),
                type="service_registration",
                data=self.registration.__dict__,
                metadata={"service_type": self.service_type}
            )
            
            await self.websocket.send(registration_message.to_json())
    ...
    async def generate_tts(self, text: str, voice: str) -> bytes:
        ...
        try:
            ...
            # Use the Zonos TTS service to generate audio
    ...
    async def send_error(self, session_id: str, error_message: str):
        """Send error message"""
        try:
            error_response = StreamingMessage(
                session_id=session_id,
                timestamp=time.time(),
                type="error",
            ...
            await self.websocket.send(error_response.to_json())
    ...
    async def send_pong(self, session_id: str):
        """Send pong response"""
        try:
            pong_response = StreamingMessage(
                session_id=session_id,
                timestamp=time.time(),
                type="pong",
            ...
            await self.websocket.send(pong_response.to_json())
